...
class FaceDataset(Dataset):
    ...
    def _plot_labels_distribution(self, save_path=None):
        ...
        plt.xlabel('Label')
        plt.ylabel('Count')
        plt.title(f'[Face] Distribution of labels for split {self.split}')
        plt.xticks(labels)
...
if __name__=="__main__":
    ...
    for split in ["train", "test"]:
        dataset = FaceDataset(dataset_path=dataset_path, split=split, resolution=224)
        ...
        dataset._plot_labels_distribution(save_path=f"_{split}_labels_face.png")