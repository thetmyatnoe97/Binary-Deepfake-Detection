...
class Face140kDataset(Dataset):
    ...
    def __getitem__(self, i):
        sample = {
            ...
            "is_real": torch.tensor(1 if self.items[i]["is_real"] else 0, dtype=torch.float32),
        }
        return sample
    ...
    def _plot_labels_distribution(self, save_path=None):
        ...
        plt.xlabel('Label')
        plt.ylabel('Count')
        plt.title(f'[Face140k] Distribution of labels for split {self.split}')
        plt.xticks(labels)
...
if __name__=="__main__":
    import random
    dataset_path = "../../datasets/face_140k"

    for split in ["train", "test"]:
        dataset = Face140kDataset(dataset_path=dataset_path, split=split, resolution=224)
        ...
        dataset._plot_labels_distribution(save_path=f"_{split}_labels_face140k.png")