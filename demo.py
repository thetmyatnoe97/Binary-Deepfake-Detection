"""
demo_comparison_minimal.py - Phase4DFD Minimal Comparison (Input + Results Only)
================================================================================
Yuan Ze University - Department of CS&E - Thesis 2026

FIXED VERSION:
  - ✅ RGB→BGR conversion for model input
  - ✅ Verbose checkpoint loading with error messages
  - ✅ Debug logit values (to catch 100% accuracy issue)
...
def load_model(model_name: str, dataset_name: str):
    """Load checkpoint with verbose error messages."""
    ...
    try:
        ...
        print(f"  [✗] Cannot import BNext4DFR: {e}")
        return None
    ...
    print(f"\n  Loading {model_name} [{dataset_name}]...")
    ...
    try:
        ...
        if 'pos_weight' in hparams:
            print(f"    ⚠️  pos_weight={hparams['pos_weight']} (high value = extreme logits)")
    except Exception:
        pass
...
if __name__ == "__main__":
    print(f"\n{'='*70}")
    print("Phase4DFD - Minimal Comparison Demo")
    ...
    print(f"  • Thesis: {len(THESIS_MODELS)} (with Attention)")
    print(f"  • Baseline: {len(BASELINE_MODELS)} (without Attention)\n")
